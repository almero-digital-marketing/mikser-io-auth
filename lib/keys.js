...
import { ensureIgnored } from 'mikser-io'
...
export async function loadOrCreateKey({ keyFile, logger }) {
    ...
    if (stored) {
        // Also on the load path: a key file that predates this guard, or one
        // whose .gitignore entry someone removed, is exactly as dangerous as a
        // new one.
        ensureIgnored(path.dirname(keyFile), path.basename(keyFile), logger)
    ...
    // Before the file exists, so it can never be staged unignored — the same
    // guard the engine puts on its durable database, and the reasoning is
    // stronger here: that file holds tokens, this one mints them. The glob
    // covers a hand-made `auth.key.bak` too, which wants ignoring just as much.
    ensureIgnored(path.dirname(keyFile), path.basename(keyFile), logger)
