...
// OAuth clients reach this server two ways, and both are needed to serve
// agents in general rather than the handful you thought to list.
//
//   config      — an operator writes the client down. Precise, auditable, and
//                 the only option for a client you want to name yourself.
//   /register   — the client registers ITSELF (RFC 7591). Required for any
//                 agent whose UI takes a URL and nothing else: it has no
//                 field to type a client_id into, so it registers or it
//                 cannot connect at all.
//
// Public clients either way: PKCE is required and there is no client_secret,
...
// Config wins over the dynamic table: an operator-declared client_id can
// never be shadowed by one somebody registered.
export function clientLookup(registered, store) {
    return (clientId) => {
        if (!clientId) return null
        return registered.get(clientId) ?? store?.getDynamicClient(clientId) ?? null
    }
}
